...
class Speech2Land(nn.Module):
    def __init__(self, args):
        ...
        self.lstm = nn.LSTM(input_size=args.audio_feature_dim, hidden_size=self.hidden_size, num_layers=self.num_layers,
                            batch_first=True, bidirectional=True, dropout=self.drop_prob)
    ...
    def forward(self, audio, landmarks, template, criterion):
        ...
        landmarks_pred = self.output_mapper(shift_emb)

        loss = criterion(landmarks_pred, landmarks - template.unsqueeze(0))
