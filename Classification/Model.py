...
class ClassificationRNN(nn.Module):
    def __init__(self, hidden_size, input_size, num_classes, device, layers):
...
class FIDRNN(nn.Module):
    def __init__(self, hidden_size, input_size, num_classes, device, layers):
        super(FIDRNN, self).__init__()
        self.layers = layers
        self.hidden_size = hidden_size
        ...
        self.fc_classification_2 = nn.Linear(2, num_classes)
    ...
    def forward(self, inputs):
        ...
        h_t = self.fc_classification(h_t)

        return self.fc_classification_2(h_t), h_t
