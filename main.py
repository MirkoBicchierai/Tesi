...
from common_function import plot_graph, import_actor, build_face, get_actor
...
def main():
    ...
    writer = SummaryWriter("TensorBoard/LABEL:" + str(num_classes) + "_HIDDEN-SIZE:" + str(
        hidden_size) + "_LR:" + str(lr) + "_COMA_40_" + datetime.now().strftime("%m-%d-%Y_%H:%M"))
    ...
    for epoch in tqdm(range(epochs)):
        ...
        for landmark_animation, label, path_gen in training_dataloader:
            for idF in reversed(range(landmark_animation[:, 1:].shape[1])):
                ...
                output = model(landmark_animation[:, idF], label, frame_generate - idF) # ,get_actor(landmark_animation, path_gen, actors_coma, name_actors_coma).to(device)
