...
class DecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, num_classes, length, device):
        ...
        self.lstm = nn.LSTMCell(output_size * 2, hidden_size)  # *3
        ...
        # self.fc_act_1 = nn.Linear(output_size, output_size * 2)
        # self.fc_act_2 = nn.Linear(output_size * 2, output_size)
    ...
    def forward(self, inputs, labels, length):  # ,actors
        ...
        # actors_trans = F.relu(self.fc_act_1(actors.flatten(-2)))
        # actors_trans = self.fc_act_2(actors_trans)

        frame = inputs.flatten(-2)
        h_t = self.fc_inh(frame)
        c_t = self.fc_inc(frame)
        frame = torch.cat([frame, encoding.to(self.device)], dim=-1)  # , actors_trans

        output = []
        for _ in range(length):
            h_t, c_t = self.lstm(frame, (h_t, c_t))
            output_ = self.fc(h_t)
            frame = torch.cat([output_, encoding.to(self.device)], dim=-1)  # , actors_trans
